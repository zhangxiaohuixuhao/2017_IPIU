...
def flip_randomly_left_right_image_with_annotation(image_tensor, annotation_tensor):
    ...
    annotation matches the image.该函数以1/2的概率执行图像和注释张量的随机翻转,对图像和注释执行或不执行翻转，
    以便进行,注释与图像匹配
    Parameters
    ----------
...
def distort_randomly_image_color(image_tensor, fast_mode=False):
    ...
    接受（宽度，高度，3）的图像张量并返回颜色失真的图像。该功能执行随机亮度，饱和度，色调，对比度变化
    Parameters
    ----------
...
def scale_randomly_image_with_annotation_with_fixed_size_output(img_tensor,
    ...
    返回一个大小的张量（output_shape，output_shape，depth）和（output_shape，output_shape，1）。
    该函数返回一个大小的张量（output_shape，output_shape，depth）它是从均匀分布中采样的因子随机缩放的
    值之间[min_relative_random_scale_change，max_realtive_random_scale_change]相乘
    通过将图像缩放到output_shape所需的因子。当重新缩放的图像不适合[output_shape]大小，图像可以填充或裁剪。而且，
    函数返回大小的缩放注释张量（output_shape，output_shape，1）。都使用最近邻插值来缩放图像张量和注释张量。
    这样做是为了保留注释标签。指定大样本时要小心,随机变量的空间 - 可能会出现锯齿效果。缩放时，此功能
    保留原始图像的宽高比。执行所有这些操作时,输出图像中会有一些区域带有空白区域 - 功能屏蔽掉,
    使用mask_out_number注释中的那些区域。总的来说，该功能执行重新缩放以获取output_shape的图像，添加随机缩放抖动，保留
    比例，掩盖出现的不必要的区域。
    Parameters
    ----------
