...
def RefineBlock(high_inputs=None,low_inputs=None):
    if low_inputs is not None:
        ...
        rcu_high1 = ResidualConvUnit(high_inputs, features=256)
        rcu_high = ResidualConvUnit(rcu_high1, features=256)
        rcu_low1 = ResidualConvUnit(low_inputs, features=256)
        rcu_low = ResidualConvUnit(rcu_low1, features=256)
...
def model(images, num_classes,weight_decay=1e-5, is_training=True):
